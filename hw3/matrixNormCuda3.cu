...
partialSum(float *input, float *output, const int N, const int Nsums) {
  ...
  // This is because the entire array might be too big and is stored into the global memory
    ...
    if (start + t < N)
      ...
      partialSum[t + ty*2*BLOCK_SIZE] = input[2 * blockIdx.x * BLOCK_SIZE + ty*2*BLOCK_SIZE + y*MAXN];
    else
    ...
    if (start + BLOCK_SIZE + t < N)
      ...
      partialSum[BLOCK_SIZE + t + ty*2*BLOCK_SIZE] = input[2 * blockIdx.x * BLOCK_SIZE + BLOCK_SIZE + ty*2*BLOCK_SIZE + y*MAXN];
    else
    ...
    for (unsigned int stride = BLOCK_SIZE; stride >= 1; stride >>= 1) {
       ...
       if (t < stride)
          ...
          partialSum[t + ty*2*BLOCK_SIZE] += partialSum[t+ stride*2*BLOCK_SIZE + ty*2*BLOCK_SIZE];
    }

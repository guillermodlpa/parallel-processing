...
partialSum(float *input, float *output, const int N, const int Noutput) {
    ...
    if (start + t < N)
      ...
      partialSum[t + y*BLOCK_SIZE] = input[start + t + y*N];
    else
    ...
    if (start + BLOCK_SIZE + t < N)
      ...
      partialSum[BLOCK_SIZE + t + y*BLOCK_SIZE] = input[start + BLOCK_SIZE + t + y*N];
    else
    ...
    if (t == 0)
      ...
      output[blockIdx.x + y*Noutput] += partialSum[y + y*BLOCK_SIZE];
}

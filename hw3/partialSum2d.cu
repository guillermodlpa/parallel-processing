...
partialSum(float *input, float *output, const int N, const int Noutput) {
    ...
    __shared__ float partialSum[4 * BLOCK_SIZE*BLOCK_SIZE];
    ...
    if (start + t < N)
       partialSum[t+y*2*BLOCK_SIZE] = input[start + t +y*N];
    else
    ...
    if (start + BLOCK_SIZE + t < N)
       partialSum[BLOCK_SIZE + t+y*2*BLOCK_SIZE] = input[start + BLOCK_SIZE + t +y*N];
    else
    ...
    for (unsigned int stride = BLOCK_SIZE; stride >= 1; stride >>= 1) {
       __syncthreads();
       if (t < stride)
          partialSum[t+y*2*BLOCK_SIZE] += partialSum[t+stride+y*2*BLOCK_SIZE];
    }

    // After the loop, the partial sum is found in partialSum[0]
    // So we have to put it in the output array
    if (t == 0)
       output[blockIdx.x + y*Noutput] = partialSum[0+y*2*BLOCK_SIZE];
}
...
{   
  ...
  int row, col;
	...
	partialSum<<< dimGrid, BLOCK_SIZE>>> (d_a, d_o, N, Noutput);
